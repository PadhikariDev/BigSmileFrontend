...
const PatientsView = ({ patients, setPatients }) => {
    ...
    // Fetch patients only if not provided
    useEffect(() => {
        ...
        bgImg.onload = () => {
            ...
            const addSection = (title, data) => {
                ...
                Object.entries(data).forEach(([k, v]) => {
                    ...
                    let value = Array.isArray(v) ? v.join(", ") : typeof v === "object" ? JSON.stringify(v) : v;
            ...
            if (patient.history.medicalReport) {
                ...
                reportImg.crossOrigin = "anonymous";
                reportImg.src = `${API}/${patient.history.medicalReport}`;
                reportImg.onload = () => {
                    if (y + 60 > 280) {
                        doc.addPage();
                        doc.addImage(bgImg, "JPEG", 0, 0, 210, 297);
                ...
                reportImg.onerror = () => doc.save(`${patient.general.name}_details.pdf`);
            } else {
                doc.save(`${patient.general.name}_details.pdf`);
            }
        };
    ...
    const downloadExcel = (patient) => {
        ...
        const addSection = (title, obj) => {
            ...
            Object.entries(obj || {}).forEach(([key, value]) => {
                const valStr = Array.isArray(value) ? value.join(", ") : typeof value === "object" ? JSON.stringify(value) : value;
                data.push({ Section: "", Field: key, Value: valStr });
            });
        };
        addSection("General", patient.general);
        addSection("History", patient.history);
        ...
        worksheet["!cols"] = [{ wch: 20 }, { wch: 30 }, { wch: 50 }];
    ...
    const handleDelete = async () => {
        ...
        try {
            await fetch(`${API}/api/patients/${selectedPatient._id}`, { method: "DELETE" });
        ...
        <div className="mt-6 bg-gray-50 p-4 rounded-2xl shadow-sm">
            ...
            <div className="flex flex-wrap gap-2 mb-4">
                ...
                <DropdownMenu>
                    ...
                    <DropdownMenuContent>
                        <DropdownMenuItem onClick={() => setGenderFilter("")}>All</DropdownMenuItem>
            ...
            <div className="overflow-x-auto rounded-xl shadow border border-gray-200">
                <table className="min-w-full text-sm text-left">
                    <thead className="bg-gradient-to-r from-orange-100 to-orange-200 text-gray-700">
                        <tr>
                            {["opdNumber", "date", "name", "age", "gender", "phone", "address"].map((col) => (
                                <th key={col} onClick={() => handleSort(col)} className="px-4 py-3 cursor-pointer select-none">
                    ...
                    <tbody className="divide-y divide-gray-200">
                        {paginatedPatients.map((patient, idx) => (
                            <tr key={patient._id} className={idx % 2 === 0 ? "bg-white hover:bg-orange-50" : "bg-gray-50 hover:bg-orange-50"}>
                                <td className="px-4 py-2">{patient.general.opdNumber}</td>
                                <td className="px-4 py-2">{new Date(patient.general.date).toLocaleDateString()}</td>
                                ...
                                <td className="px-4 py-2 text-center">
                                    <DropdownMenu>
                                        ...
                                        <DropdownMenuContent>
                                            ...
                                            <DropdownMenuItem onClick={() => downloadPDF(patient)}>Download PDF</DropdownMenuItem>
                                            <DropdownMenuItem onClick={() => downloadExcel(patient)}>Download Excel</DropdownMenuItem>
                                            <DropdownMenuItem
                                                onClick={() => {
                                                    setSelectedPatient(patient);
                ...
                <DialogContent className="max-h-[80vh] overflow-y-auto">
                    ...
                    {selectedPatient && (
                        <div className="space-y-4 text-sm">
                            {["general", "history", "prescription", "evaluation", "examine"].map((section) => (
                                <div key={section}>
                                    <h3 className="font-bold text-gray-700 capitalize">{section}</h3>
                                    {renderDetails(selectedPatient[section])}
                                </div>
                            ))}
                        </div>
                    )}
                    <DialogFooter>
                        <Button onClick={() => downloadPDF(selectedPatient)}>Download PDF</Button>
                    </DialogFooter>
                </DialogContent>
            </Dialog>
            ...
            <Dialog open={isDeleteOpen} onOpenChange={setIsDeleteOpen}>
                <DialogContent>
                    ...
                    <p className="text-sm text-gray-600">Do you really want to delete this patient? This action is permanent.</p>
                    <DialogFooter>
